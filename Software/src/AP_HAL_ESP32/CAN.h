...
#include <./src/AP_HAL/CAN.h>
#include <pthread.h>
#include <semaphore.h>

#include "bxcan.h"
#include "./src/AP_HAL/utility/RingBuffer.h"
...
public:
    BusEvent(ESP32CANManager& can_driver);
    ~BusEvent();
...
class ESP32CAN: public AP_HAL::CAN {
    struct Timings {
        uint16_t prescaler;
        uint8_t sjw;
        uint8_t bs1;
...
public:
    ...
    ESP32CAN(bxcan::CanType* can, BusEvent* update_event, uint8_t self_index, uint8_t rx_queue_capacity) :
...
class ESP32CANManager: public AP_HAL::CANManager, public uavcan::ICanDriver {
    BusEvent update_event_;
    ESP32CAN if0_;
    ESP32CAN if1_;
    ...
    ESP32CAN* ifaces[CAN_STM32_NUM_IFACES];
    uint8_t _ifaces_num;
...
public:
     ...
     */
    int init(const uint32_t bitrate, const ESP32CAN::OperatingMode mode, uint8_t can_number);

    virtual ESP32CAN* getIface(uint8_t iface_index) override;
    ESP32CAN* getIface_out_to_in(uint8_t iface_index);
