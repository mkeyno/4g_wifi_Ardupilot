...
BusEvent::BusEvent(ESP32CANManager& can_driver) :
    _signal(0)
{
...
static void handleTxInterrupt(uint8_t iface_index)
{
    ...
    for (uint8_t i = 0; i < MAX_NUMBER_OF_CAN_DRIVERS; i++) {
        ...
        ESP32CAN* iface = ((ESP32CANManager*) hal.can_mgr[i])->getIface_out_to_in(iface_index);
        if (iface != nullptr) {
...
const uint32_t ESP32CAN::TSR_ABRQx[ESP32CAN::NumTxMailboxes] = { bxcan::TSR_ABRQ0, bxcan::TSR_ABRQ1, bxcan::TSR_ABRQ2 };

int ESP32CAN::computeTimings(const uint32_t target_bitrate, Timings& out_timings)
{
    if (target_bitrate < 1) {
    ...
    if (max_quanta_per_bit > (MaxBS1 + MaxBS2)) {
        if (AP::can().get_debug_level(self_index_) >= 1) {
            printf("ESP32CAN::computeTimings max_quanta_per_bit problem\n\r");
        }
    }
    ...
    struct BsPair {
                ...
                uint16_t(1000 * (1 + bs1) / (1 + bs1 + bs2)))
        {
            if (bs1_bs2_sum <= arg_bs1) {
                AP_HAL::panic("ESP32CAN::computeTimings bs1_bs2_sum <= arg_bs1");
            }
        }
    ...
    if (solution.sample_point_permill > MaxSamplePointLocation || !solution.isValid()) {
        ...
        if (!solution.isValid())
        {
            printf("ESP32CAN::computeTimings second solution invalid\n\r");
            return -ErrLogic;
        }
    }
    ...
    if ((target_bitrate != (pclk / (prescaler * (1 + solution.bs1 + solution.bs2)))) || !solution.isValid()) {
        if (AP::can().get_debug_level(self_index_) >= 1) {
            printf("ESP32CAN::computeTimings target_bitrate error\n\r");
        }
        return -ErrLogic;
    }

    if (AP::can().get_debug_level(self_index_) >= 2) {
        printf("ESP32CAN::computeTimings Timings: quanta/bit: %d, sample point location: %.1f%%\n\r",
...
int16_t ESP32CAN::send(const uavcan::CanFrame& frame, uavcan::MonotonicTime tx_deadline, uavcan::CanIOFlags flags)
{
...
int16_t ESP32CAN::receive(uavcan::CanFrame& out_frame, uavcan::MonotonicTime& out_ts_monotonic,
...
int16_t ESP32CAN::configureFilters(const uavcan::CanFilterConfig* filter_configs, uint16_t num_configs)
{
...
bool ESP32CAN::waitMsrINakBitStateChange(bool target_state)
{
...
int ESP32CAN::init(const uint32_t bitrate, const OperatingMode mode)
{
    ...
    if (!waitMsrINakBitStateChange(true)) {
        if (AP::can().get_debug_level(self_index_) >= 1) {
            printf("ESP32CAN::init MSR INAK not set\n\r");
        }
    ...
    if (AP::can().get_debug_level(self_index_) >= 2) {
        printf("ESP32CAN::init Timings: presc=%u sjw=%u bs1=%u bs2=%u\n\r", unsigned(timings.prescaler),
    ...
    if (!waitMsrINakBitStateChange(false)) {
        if (AP::can().get_debug_level(self_index_) >= 1) {
            printf("ESP32CAN::init MSR INAK not cleared\n\r");
        }
        can_->MCR = bxcan::MCR_RESET;
...
void ESP32CAN::handleTxMailboxInterrupt(uint8_t mailbox_index, bool txok, const uint64_t utc_usec)
{
...
void ESP32CAN::handleTxInterrupt(const uint64_t utc_usec)
{
...
void ESP32CAN::handleRxInterrupt(uint8_t fifo_index, uint64_t utc_usec)
{
    if (fifo_index >= 2) {
        return;
    }
...
void ESP32CAN::pollErrorFlagsFromISR()
{
...
void ESP32CAN::discardTimedOutTxMailboxes(uavcan::MonotonicTime current_time)
{
    CriticalSectionLocker lock;
...
bool ESP32CAN::canAcceptNewTxFrame(const uavcan::CanFrame& frame) const
{
    /*
...
bool ESP32CAN::isRxBufferEmpty() const
{
    CriticalSectionLocker lock;
    return rx_queue_.available() == 0;
}

uint64_t ESP32CAN::getErrorCount() const
{
    CriticalSectionLocker lock;
    return error_cnt_;
...
unsigned ESP32CAN::getRxQueueLength() const
{
    CriticalSectionLocker lock;
    return rx_queue_.available();
}

bool ESP32CAN::hadActivity()
{
    CriticalSectionLocker lock;
...
bool ESP32CAN::begin(uint32_t bitrate)
{
...
bool ESP32CAN::is_initialized()
{
    return initialized_;
}

int32_t ESP32CAN::available()
{
    if (initialized_) {
...
ESP32CANManager::ESP32CANManager() :
...
{
    uavcan::StaticAssert<(CAN_STM32_RX_QUEUE_SIZE <= ESP32CAN::MaxRxQueueCapacity)>::check();
...
uavcan::CanSelectMasks ESP32CANManager::makeSelectMasks(const uavcan::CanFrame* (&pending_tx)[uavcan::MaxCanIfaces]) const
{
...
bool ESP32CANManager::hasReadableInterfaces() const
{
    bool ret = false;
...
int16_t ESP32CANManager::select(uavcan::CanSelectMasks& inout_masks,
...
void ESP32CANManager::initOnce(uint8_t can_number)
{
    {
        CriticalSectionLocker lock;
        if (can_number == 0) {
...
int ESP32CANManager::init(const uint32_t bitrate, const ESP32CAN::OperatingMode mode, uint8_t can_number)
{
    ...
    if (AP::can().get_debug_level(can_number) >= 2) {
        printf("ESP32CANManager::init Bitrate %lu mode %d bus %d\n\r", static_cast<unsigned long>(bitrate),
    ...
    if (!initialized_once[can_number]) {
        ...
        if (AP::can().get_debug_level(can_number) >= 2) {
            printf("ESP32CANManager::init First initialization bus %d\n\r", static_cast<int>(can_number));
        }
    ...
    if (can_number == 0) {
        if (AP::can().get_debug_level(0) >= 2) {
            printf("ESP32CANManager::init Initing iface 0...\n\r");
        }
    ...
    if (can_number == 1) {
        if (AP::can().get_debug_level(1) >= 2) {
            printf("ESP32CANManager::init Initing iface 1...\n\r");
        }
    ...
    if (AP::can().get_debug_level(can_number) >= 2) {
        printf("ESP32CANManager::init CAN drv init OK, res = %d\n\r", res);
    }

    return res;
}

ESP32CAN* ESP32CANManager::getIface(uint8_t iface_index)
{
...
ESP32CAN* ESP32CANManager::getIface_out_to_in(uint8_t iface_index)
{
...
bool ESP32CANManager::hadActivity()
{
    bool ret = false;
...
bool ESP32CANManager::begin(uint32_t bitrate, uint8_t can_number)
{
    // Try to init outside physical interface 'can_number'
    if (init(bitrate, ESP32CAN::OperatingMode::NormalMode, can_number) >= 0) {
        return true;
    }
...
bool ESP32CANManager::is_initialized()
{
    return initialized_;
}

void ESP32CANManager::initialized(bool val)
{
    initialized_ = val;
}
