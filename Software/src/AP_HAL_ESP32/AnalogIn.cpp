...
#define ESP32_VOLTAGE_SCALING (3.3f/4096.0f)
...
ESP32AnalogSource::ESP32AnalogSource(int16_t pin, float initial_value) :
	_pin(pin),
    _stop_pin(-1),
    _settle_time_ms(0),
    ...
    _sum_ratiometric(0)
{
#ifdef ESP32_ANALOG_VCC_5V_PIN
    if (_pin == ANALOG_INPUT_BOARD_VCC) {
        _pin = ESP32_ANALOG_VCC_5V_PIN;
    }
#endif
}

void ESP32AnalogSource::set_stop_pin(uint8_t p)
{ 
    _stop_pin = p; 
}

float ESP32AnalogSource::read_average() 
{
...
float ESP32AnalogSource::read_latest() 
{
 ...
 */
float ESP32AnalogSource::_pin_scaler(void)
{
    float scaling = ESP32_VOLTAGE_SCALING;
 ...
 */
float ESP32AnalogSource::voltage_average()
{
...
/*
  ...
  the 5V rail
 */
float ESP32AnalogSource::voltage_average_ratiometric()
{
    voltage_average();
...
/*
  return voltage in Volts
 */
float ESP32AnalogSource::voltage_latest()
{
    return _pin_scaler() * read_latest();
}

void ESP32AnalogSource::set_pin(uint8_t pin)
{
    if (_pin == pin) {
        return;
    }
 ...
 */
void ESP32AnalogSource::_add_value(float v, float vcc5V)
{
    WITH_SEMAPHORE(_semaphore);
 ...
 */
void ESP32AnalogIn::next_stop_pin(void)
{
    ...
    // one stop pin
    for (uint8_t i=1; i <= ESP32_ANALOG_MAX_CHANNELS; i++) {
        uint8_t idx = (_current_stop_pin_i + i) % ESP32_ANALOG_MAX_CHANNELS;
        ESP32::ESP32AnalogSource *c = _channels[idx];
        if (c && c->_stop_pin != -1) {
            ...
            // set all others low
            for (uint8_t j=0; j<ESP32_ANALOG_MAX_CHANNELS; j++) {
                ESP32::ESP32AnalogSource *c2 = _channels[j];
...
/*
  called at 1kHz
 */
void ESP32AnalogIn::_timer_tick(void)
{
    if (_adc_fd == -1) {
        // not initialised yet
        return;
    ...
    struct adc_msg_s buf_adc[ESP32_ANALOG_MAX_CHANNELS];
    ...
    if (ret > 0) {
        ...
        for (uint8_t i=0; i<ret/sizeof(buf_adc[0]); i++) {
            ...
            for (uint8_t j=0; j<ESP32_ANALOG_MAX_CHANNELS; j++) {
                ESP32::ESP32AnalogSource *c = _channels[j];
        ...
        if (orb_check(_battery_handle, &updated) == 0 && updated) {
            ...
            if (battery.timestamp != _battery_timestamp) {
                ...
                for (uint8_t j=0; j<ESP32_ANALOG_MAX_CHANNELS; j++) {
                    ...
                    if (c->_pin == ESP32_ANALOG_ORB_BATTERY_VOLTAGE_PIN) {
                        c->_add_value(battery.voltage_v / ESP32_VOLTAGE_SCALING, 0);
                    }
                    if (c->_pin == ESP32_ANALOG_ORB_BATTERY_CURRENT_PIN) {
                        // scale it back to voltage, knowing that the
                        // px4io code scales by 90.0/5.0
                        c->_add_value(battery.current_a * (5.0f/90.0f) / ESP32_VOLTAGE_SCALING, 0);
                    }
                }
            }
        }
        ...
        if (orb_check(_servorail_handle, &updated) == 0 && updated) {
            ...
            if (servorail.timestamp != _servorail_timestamp) {
                ...
                for (uint8_t j=0; j<ESP32_ANALOG_MAX_CHANNELS; j++) {
                    ESP32::ESP32AnalogSource *c = _channels[j];
                    if (c == nullptr) continue;
                    if (c->_pin == ESP32_ANALOG_ORB_SERVO_VOLTAGE_PIN) {
                        c->_add_value(servorail.voltage_v / ESP32_VOLTAGE_SCALING, 0);
                    }
                    if (c->_pin == ESP32_ANALOG_ORB_SERVO_VRSSI_PIN) {
                        c->_add_value(servorail.rssi_v / ESP32_VOLTAGE_SCALING, 0);
                    }
                }
            }
        }
...
AP_HAL::AnalogSource* ESP32AnalogIn::channel(int16_t pin) 
{
    for (uint8_t j=0; j<ESP32_ANALOG_MAX_CHANNELS; j++) {
        if (_channels[j] == nullptr) {
            _channels[j] = new ESP32AnalogSource(pin, 0.0f);
            return _channels[j];
        }
    }
