...
uint8_t ESP32::ESP32_I2C::instance;
pthread_mutex_t ESP32::ESP32_I2C::instance_lock;
 ...
 */    
ESP32_I2C::ESP32_I2C(uint8_t bus) :
  I2C(devname, devpath, map_bus_number(bus), 0, 100000UL)
{}

/*
  map ArduPilot bus numbers to ESP32 bus numbers
 */    
uint8_t ESP32_I2C::map_bus_number(uint8_t bus) const
{
    switch (bus) {
    case 0:
        // map to internal bus
#ifdef ESP32_I2C_BUS_ONBOARD
        return ESP32_I2C_BUS_ONBOARD;
#else
        return 0;
#endif
...
#ifdef ESP32_I2C_BUS_EXPANSION
        return ESP32_I2C_BUS_EXPANSION;
#else
        return 1;
#endif
    case 2:
        // map to expansion bus 2
#ifdef ESP32_I2C_BUS_EXPANSION1
        return ESP32_I2C_BUS_EXPANSION1;
#else
        return 2;
#endif
    }
...
/*
  implement wrapper for ESP32 I2C driver
 */
bool ESP32_I2C::do_transfer(uint8_t address, const uint8_t *send, uint32_t send_len, uint8_t *recv, uint32_t recv_len, bool split_transfers)
{
    set_address(address);
    if (!init_done) {
...
{
    perf_begin(perf);
    bool ret = _ESP32dev.do_transfer(_address, send, send_len, recv, recv_len, _split_transfers);
    perf_end(perf);
    return ret;
}
