...
#include <modules/ESP32iofirmware/protocol.h>
...
void ESP32GPIO::init()
{
    ...
    _gpio_fmu_fd = open(ESP32FMU_DEVICE_PATH, 0);
    if (_gpio_fmu_fd == -1) {
    ...
    _gpio_io_fd = open(ESP32IO_DEVICE_PATH, O_RDWR);
    if (_gpio_io_fd == -1) {
        hal.console->printf("GPIO: Unable to open ESP32io\n");
    }
#endif
}

void ESP32GPIO::pinMode(uint8_t pin, uint8_t output)
{
    switch (pin) {
    case ESP32_GPIO_FMU_SERVO_PIN(0) ... ESP32_GPIO_FMU_SERVO_PIN(5):
        uint32_t pinmask = 1U<<(pin-ESP32_GPIO_FMU_SERVO_PIN(0));
        if (output) {
...
uint8_t ESP32GPIO::read(uint8_t pin) {
    switch (pin) {

#ifdef GPIO_EXT_1
        case ESP32_GPIO_EXT_FMU_RELAY1_PIN: {
            uint32_t relays = 0;
...
#ifdef GPIO_EXT_2
        case ESP32_GPIO_EXT_FMU_RELAY2_PIN: {
            uint32_t relays = 0;
            ioctl(_gpio_fmu_fd, GPIO_GET, (unsigned long)&relays);
        ...
        case ESP32_GPIO_EXT_IO_RELAY1_PIN: {
            uint32_t relays = 0;
            if (_gpio_io_fd == -1) {
                return LOW;
            }
            ioctl(_gpio_io_fd, GPIO_GET, (unsigned long)&relays);
            return (relays & ESP32IO_P_SETUP_RELAYS_POWER1)?HIGH:LOW;
        }
#endif

#ifdef PX4IO_P_SETUP_RELAYS_POWER2
        case ESP32_GPIO_EXT_IO_RELAY2_PIN: {
            uint32_t relays = 0;
            if (_gpio_io_fd == -1) {
                return LOW;
            }
        ...
        case ESP32_GPIO_EXT_IO_ACC1_PIN: {
            uint32_t relays = 0;
            if (_gpio_io_fd == -1) {
                return LOW;
            }
            ioctl(_gpio_io_fd, GPIO_GET, (unsigned long)&relays);
            return (relays & ESP32IO_P_SETUP_RELAYS_ACC1)?HIGH:LOW;
        }
#endif

#ifdef PX4IO_P_SETUP_RELAYS_ACC2
        case ESP32_GPIO_EXT_IO_ACC2_PIN: {
            uint32_t relays = 0;
            if (_gpio_io_fd == -1) {
                return LOW;
            }
            ioctl(_gpio_io_fd, GPIO_GET, (unsigned long)&relays);
            return (relays & ESP32IO_P_SETUP_RELAYS_ACC2)?HIGH:LOW;
        }
#endif
    ...
    case ESP32_GPIO_FMU_SERVO_PIN(0) ... ESP32_GPIO_FMU_SERVO_PIN(5): {
            uint32_t relays = 0;
            ioctl(_gpio_fmu_fd, GPIO_GET, (unsigned long)&relays);
            return (relays & (1U<<(pin-ESP32_GPIO_FMU_SERVO_PIN(0))))?HIGH:LOW;
        }
    }
    return LOW;
}

void ESP32GPIO::write(uint8_t pin, uint8_t value)
{
    switch (pin) {
        ...
        case ESP32_GPIO_EXT_IO_RELAY1_PIN:
            if (_gpio_io_fd != -1) {
                ioctl(_gpio_io_fd, value==LOW?GPIO_CLEAR:GPIO_SET, ESP32IO_P_SETUP_RELAYS_POWER1);
            }
            break;
#endif
        ...
        case ESP32_GPIO_EXT_IO_RELAY2_PIN:
            if (_gpio_io_fd != -1) {
                ioctl(_gpio_io_fd, value==LOW?GPIO_CLEAR:GPIO_SET, ESP32IO_P_SETUP_RELAYS_POWER2);
            }
            break;
#endif
        ...
        case ESP32_GPIO_EXT_IO_ACC1_PIN:
            if (_gpio_io_fd != -1) {
                ioctl(_gpio_io_fd, value==LOW?GPIO_CLEAR:GPIO_SET, ESP32IO_P_SETUP_RELAYS_ACC1);
            }
            break;
#endif
        ...
        case ESP32_GPIO_EXT_IO_ACC2_PIN:
            if (_gpio_io_fd != -1) {
                ioctl(_gpio_io_fd, value==LOW?GPIO_CLEAR:GPIO_SET, ESP32IO_P_SETUP_RELAYS_ACC2);
            }
            break;
#endif

    case ESP32_GPIO_FMU_SERVO_PIN(0) ... ESP32_GPIO_FMU_SERVO_PIN(5):
        ioctl(_gpio_fmu_fd, value==LOW?GPIO_CLEAR:GPIO_SET, 1U<<(pin-ESP32_GPIO_FMU_SERVO_PIN(0)));
        break;
    }
}
...
AP_HAL::DigitalSource* ESP32GPIO::channel(uint16_t n) {
    return new ESP32DigitalSource(0);
}

/* Interrupt interface: */
AP_HAL::GPIO::irq_handler_fn_t ESP32GPIO::pin_handlers[6] = {};

int ESP32GPIO::irq_handler_gpio0(int irq, void *context)
{
    irq_handler(0);
    return 0;
}
int ESP32GPIO::irq_handler_gpio1(int irq, void *context)
{
    irq_handler(1);
    return 0;
}
int ESP32GPIO::irq_handler_gpio2(int irq, void *context)
{
    irq_handler(2);
    return 0;
}
int ESP32GPIO::irq_handler_gpio3(int irq, void *context)
{
    irq_handler(3);
    return 0;
}
int ESP32GPIO::irq_handler_gpio4(int irq, void *context)
{
    irq_handler(4);
    return 0;
}
int ESP32GPIO::irq_handler_gpio5(int irq, void *context)
{
    irq_handler(5);
    return 0;
}

void ESP32GPIO::irq_handler(uint8_t handler)
{
...
uint32_t ESP32GPIO::pin_to_gpio(uint8_t pin)
{
...
bool ESP32GPIO::attach_interrupt(uint8_t pin,
                               irq_handler_fn_t fn,
 ...
 */
bool ESP32GPIO::usb_connected(void)
{
    /*
...
ESP32DigitalSource::ESP32DigitalSource(uint8_t v) :
    _v(v)
{}

void ESP32DigitalSource::mode(uint8_t output)
{}

uint8_t ESP32DigitalSource::read() {
    return _v;
}

void ESP32DigitalSource::write(uint8_t value) {
    _v = value;
}

void ESP32DigitalSource::toggle() {
    _v = !_v;
}
