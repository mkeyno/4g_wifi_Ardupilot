...
#include <./src/AP_HAL/utility/RCOutput_Tap.h>
#include <./src/AP_HAL_Empty/AP_HAL_Empty.h>
#include <./src/AP_HAL_Empty/AP_HAL_Empty_Private.h>
...
static ESP32Scheduler schedulerInstance;
static ESP32Storage storageDriver;
static ESP32RCInput rcinDriver;
#if defined(CONFIG_ARCH_BOARD_AEROFC_V1)
static RCOutput_Tap rcoutDriver;
#else
static ESP32RCOutput rcoutDriver;
#endif
static ESP32AnalogIn analogIn;
static ESP32Util utilInstance;
static ESP32GPIO gpioDriver;

static ESP32::I2CDeviceManager i2c_mgr_instance;
static ESP32::SPIDeviceManager spi_mgr_instance;
...
static ESP32UARTDriver uartADriver(UARTA_DEFAULT_DEVICE, "APM_uartA");
static ESP32UARTDriver uartBDriver(UARTB_DEFAULT_DEVICE, "APM_uartB");
static ESP32UARTDriver uartCDriver(UARTC_DEFAULT_DEVICE, "APM_uartC");
static ESP32UARTDriver uartDDriver(UARTD_DEFAULT_DEVICE, "APM_uartD");
static ESP32UARTDriver uartEDriver(UARTE_DEFAULT_DEVICE, "APM_uartE");
static ESP32UARTDriver uartFDriver(UARTF_DEFAULT_DEVICE, "APM_uartF");
...
bool _ESP32_thread_should_exit = false;        /**< Daemon exit flag */
static bool thread_running = false;        /**< Daemon status flag */
static int daemon_task;                /**< Handle of daemon task / thread */
bool ESP32_ran_overtime;
...
{
    hal_ESP32_set_priority(APM_OVERTIME_PRIORITY);
    ESP32_ran_overtime = true;
}
...
static int main_loop(int argc, char **argv)
{
    ...
    while (!_ESP32_thread_should_exit) {
...
void HAL_ESP32::run(int argc, char * const argv[], Callbacks* callbacks) const
{
    ...
    for (i=0; i<argc; i++) {
        if (strcmp(argv[i], "start") == 0) {
            ...
            _ESP32_thread_should_exit = false;
            daemon_task = ESP32_task_spawn_cmd(SKETCHNAME,
                                             SCHED_FIFO,
                                             APM_MAIN_PRIORITY,
        ...
        if (strcmp(argv[i], "stop") == 0) {
            _ESP32_thread_should_exit = true;
            exit(0);
        }
 
        if (strcmp(argv[i], "status") == 0) {
            if (_ESP32_thread_should_exit && thread_running) {
                printf("\t%s is exiting\n", SKETCHNAME);
